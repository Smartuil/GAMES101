...
void rst::rasterizer::draw(pos_buf_id pos_buffer, ind_buf_id ind_buffer, col_buf_id col_buffer, Primitive type, bool InMSAA)
{
	...
	if (MSAA) {
		...
		for (int x = min_x; x <= max_x; x++) {
			for (int y = min_y; y <= max_y; y++) {
				...
				for (int i = 0; i < 4; i++) {
					// 小点是否在三角形内
                    //std::cout<<insideTriangle((float)x + pos[i][0], (float)y + pos[i][1], t.v);
...
//Screen space rasterization
void rst::rasterizer::rasterize_triangle2(const Triangle& t, bool InMSAA) {
    auto v = t.toVector4();
    ...
    // std::floor -->向下取整数,std::ceil   -->向上取整数：
    min_x = (int)std::floor(min_x);
    max_x = (int)std::ceil(max_x);
    min_y = (int)std::floor(min_y);
    max_y = (int)std::ceil(max_y);
...
void rst::rasterizer::clear(rst::Buffers buff)
{
    if ((buff & rst::Buffers::Color) == rst::Buffers::Color)
    {
        ...
        std::fill(frame_buf2.begin(), frame_buf2.end(), Eigen::Vector3f{0, 0, 0});
    }
